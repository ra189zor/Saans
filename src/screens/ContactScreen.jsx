 ...
 */
export default function ContactScreen({ lang, onLangChange, onAnswer, onBack }) {
  return (
    <Screen>
      ...
      <main className={`${COLUMN} flex flex-1 flex-col pt-6 md:pt-10`}>
        ...
        <div className="flex flex-1 items-center">
          <h1 className="text-[1.75rem] leading-snug font-medium text-balance text-fg md:text-[2.5rem] md:leading-[1.25]">
            Close or household TB contact in the previous 12 months?
          </h1>
        </div>
      </main>
      ...
      <footer className={`${COLUMN} pt-8 pb-8 md:pt-10 md:pb-12`}>
        <div className="flex flex-col gap-3 md:gap-4">
          <ChoiceButton onClick={() => onAnswer(true)}>YES</ChoiceButton>
          <ChoiceButton onClick={() => onAnswer(false)}>NO</ChoiceButton>
        </div>
      </footer>
    </Screen>
  )
